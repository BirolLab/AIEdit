...
static const int NTHASH_HASH_NUM_MAX = std::numeric_limits<NTHASH_HASH_NUM_TYPE>::max();
...
class NtHash
{
  ...
  public:
    ...
    void change_seq(const std::string& new_seq, bool reset = true)
    {
        seq = new_seq.data();
...
class SeedNtHash
{
  ...
  public:
    ...
    void change_seq(std::string& seq, bool reset = true) { nthash.change_seq(seq, reset); }
...
#define BTLLIB_NTHASH_INIT(CLASS, NTHASH_CALL, MEMBER_PREFIX)                       \
    ...
    {                                                                               \
        ...
        unsigned posN = 0;                                                          \
        while ((MEMBER_PREFIX pos < MEMBER_PREFIX seq_len - MEMBER_PREFIX k + 1) && \
               !(NTHASH_CALL)) {                                                    \
...
#define BTLLIB_NTHASH_ROLL(CLASS, FN_DECL, NTHASH_CALL, MEMBER_PREFIX)                           \
    ...
    {                                                                                            \
        ...
        if (SEED_TAB[(unsigned char)(MEMBER_PREFIX seq[MEMBER_PREFIX pos + MEMBER_PREFIX k])] == \
            SEED_N) {                                                                            \
...
#define BTLLIB_NTHASH_ROLL_BACK(CLASS, FN_DECL, NTHASH_CALL, MEMBER_PREFIX)                  \
    ...
    {                                                                                        \
        ...
        if (SEED_TAB[(unsigned char)(MEMBER_PREFIX seq[MEMBER_PREFIX pos - 1])] == SEED_N) { \
            MEMBER_PREFIX pos -= MEMBER_PREFIX k;                                            \
            return init();                                                                   \
        }                                                                                    \
        NTHASH_CALL /* NOLINT(bugprone-macro-parentheses) */                                 \
...
BTLLIB_NTHASH_INIT(
  NtHash, ntmc64(seq + pos, k, hash_num, forward_hash, reverse_hash, posN, hashes_array.get()), )
BTLLIB_NTHASH_ROLL(
  NtHash,
  roll(),
...
BTLLIB_NTHASH_ROLL_BACK(
  ...
  roll_back(),
  ntmc64l(
    seq[pos + k - 1], seq[pos - 1], k, hash_num, forward_hash, reverse_hash, hashes_array.get());
  , )
BTLLIB_NTHASH_PEEK(
  NtHash,
  peek(),
...
BTLLIB_NTHASH_PEEK(
...

  {
      ...
      std::memcpy(fh_no_monomers_tmp.get(), forward_hash.get(), blocks.size() * sizeof(uint64_t));
      std::memcpy(rh_no_monomers_tmp.get(), reverse_hash.get(), blocks.size() * sizeof(uint64_t));
