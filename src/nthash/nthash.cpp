...
NtHash::NtHash(const char* seq, size_t seq_len, unsigned hash_num, unsigned k, size_t pos)
  : seq(seq)
  , seq_len(seq_len)
  , hash_num(hash_num)
  , k(k)
  ...
  , hashes_array(new uint64_t[hash_num])
{
    ...
    btllib::check_error(hash_num > NTHASH_HASH_NUM_MAX,
                        "NtHash: passed number of hashes (" + std::to_string(hash_num) +
                          ") is larger than allowed (" + std::to_string(NTHASH_HASH_NUM_MAX) +
                          ").");
...
NtHash::NtHash(const std::string& seq, unsigned hash_num, unsigned k, size_t pos)
...
{
    std::memcpy(hashes_array.get(), nthash.hashes_array.get(), hash_num * sizeof(uint64_t));
}

BlindNtHash::BlindNtHash(const char* seq, size_t seq_len, unsigned hash_num, unsigned k, size_t pos)
  ...
  , hashes_array(new uint64_t[hash_num])
{
    btllib::check_error(k == seq_len,
                        "BlindNtHash: passed sequence length (" + std::to_string(seq_len) +
                          ") is not equal to k (" + std::to_string(k) + ").");
    btllib::check_error(k > NTHASH_K_MAX,
                        "BlindNtHash: passed k value (" + std::to_string(k) +
                          ") is larger than allowed (" + std::to_string(NTHASH_K_MAX) + ").");
    btllib::check_error(hash_num > NTHASH_HASH_NUM_MAX,
                        "BlindNtHash: passed number of hashes (" + std::to_string(hash_num) +
                          ") is larger than allowed (" + std::to_string(NTHASH_HASH_NUM_MAX) +
                          ").");
    btllib::check_warning(hash_num >= k,
...
BlindNtHash::BlindNtHash(const std::string& seq, unsigned hash_num, unsigned k, size_t pos)
...
void check_seeds(const std::vector<std::string>& seeds, unsigned k)
{
    for (const auto& seed : seeds) {
        ...
        btllib::check_warning(
          seed != reversed,
          "Seed " + seed + " is not symmetric, reverse-complement hashing will be inconsistent");
    }
}
...
void parse_seeds(const std::vector<std::string>& seed_strings,
...
void parsed_seeds_to_blocks(const std::vector<SpacedSeed>& seeds,
                            unsigned k,
                            std::vector<SpacedSeedBlocks>& out_blocks,
                            std::vector<SpacedSeedMonomers>& out_monomers)
{
...
void NtHash::sub(const std::vector<unsigned>& positions,
...
void BlindNtHash::sub(const std::vector<unsigned>& positions,
                      const std::vector<unsigned char>& new_bases)
{
    sub_hash(forward_hash,
             reverse_hash,
...
std::vector<std::vector<uint64_t>> SeedNtHash::peek_window(unsigned n)
{
    ...
    std::unique_ptr<uint64_t[]> hval_temp(new uint64_t[nthash.hash_num]);
    std::memcpy(fh_no_monomers_tmp.get(), fh_no_monomers.get(), blocks.size() * sizeof(uint64_t));
    std::memcpy(rh_no_monomers_tmp.get(), rh_no_monomers.get(), blocks.size() * sizeof(uint64_t));
    std::memcpy(forward_hash_tmp.get(), forward_hash.get(), blocks.size() * sizeof(uint64_t));
    std::memcpy(reverse_hash_tmp.get(), reverse_hash.get(), blocks.size() * sizeof(uint64_t));
