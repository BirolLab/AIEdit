...
def _make_kmer_model(seq: str, seeds: list[str]) -> core.BFKmerModel:
    ...
    bf_seeds.insert(seq)
    bf_kmers = btllib.KmerBloomFilter(1024 * 1024, 7, len(seeds[0]))
    bf_kmers.insert(seq)
...
def _get_mismatch_signature(
    ...
    interface = core.ModelInterface("".join(edited), 1, end, 0, max_indels, kmer_model)
    return buffer2d_to_tensor(interface.get_signature())


def _get_insertion_signature(
    ...
    interface = core.ModelInterface(edited, 1, num_ins + k, 0, max_indels, kmer_model)
    return buffer2d_to_tensor(interface.get_signature())


def _get_deletion_signature(
    seq: str, num_del: int, max_indels: int, kmer_model: core.BFKmerModel
) -> torch.FloatTensor:
    k = kmer_model.get_kmer_size()
    edited = seq[:k] + seq[k + num_del :]
    interface = core.ModelInterface(edited, 1, k, 0, max_indels, kmer_model)
    return buffer2d_to_tensor(interface.get_signature())
